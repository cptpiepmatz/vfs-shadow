...
pub fn main() {
    ...
    assert_eq!(
        ...
        include_bytes!("./vfs/config.toml")
    );
    assert_eq!(
        read_all(fs.open_file("/data.json").unwrap()),
        include_bytes!("./vfs/data.json")
    );
    assert_eq!(
        read_all(fs.open_file("/README.md").unwrap()),
        include_bytes!("./vfs/README.md")
    );
